...
export default class Predictor {
	 ...
	 * @param {Function} opts.errorFunction
	 * @param {Function} opts.activationFunction
	 * */
	constructor(opts = {}) {
		let {
			layers,
	...
	/**
	 * Вернуть прогноз (классифицировать)
	 *
	 * @param {Array} vec - массив признаков
	 *
	 * @return {Array}
	 * */
	predict(vec) {
	 ...
	 * @param {Array} options.data - обучающая выборка
	 * @param {Function=} options.onLog - журналирование. функция вызывается на каждой итерации по обучающей выборке
	 * @param {Number=1} options.epochs - количество повторений
	 * @param {Number=1} options.learningRate - коэфициент обучения (смещения, шага) градиентного спуска
	 *
	 * @return undefined
	 * */
	train(options) {
	...
	/**
	 * Вернуть состояние модели
	 *
	 * @return {Object} - { weights, biases, nodes }
	 * */
	getState() {
